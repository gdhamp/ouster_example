...
using LaserScanOS1 = ouster_ros::OS1::LaserScanOS1;
using Point2dOS1 = ouster_ros::OS1::Point2dOS1;

namespace OS1 = ouster::OS1;

int main(int argc, char** argv) {
    ros::init(argc, argv, "os1_laserscan_node");
    ros::NodeHandle nh("~");
    ...
    auto lidar_pub = nh.advertise<sensor_msgs::LaserScan>("laserscan", 10);
    auto imu_pub = nh.advertise<sensor_msgs::Imu>("imu", 100);

    auto xyz_lut = OS1::make_closest_xyz_lut(W, H, cfg.response.beam_azimuth_angles,
                                     cfg.response.beam_altitude_angles);

    LaserScanOS1 scan{W, H}; // TBD this may need to change after deciding what points to keep.
    auto it = scan.begin();
